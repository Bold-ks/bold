    ...
    <section className="relative h-[70vh] flex items-center justify-center overflow-hidden">
      ...
      <div className={`relative z-10 text-center px-6 ${!loaded ? 'opacity-0' : 'opacity-100'} transition-opacity duration-500`}>
        <motion.h1
          ...
          className={`text-4xl md:text-7xl font-light tracking-tight mb-4 ${isDark ? 'text-white' : 'text-black'}`}
        >
          {title}
        </motion.h1>
        <motion.p
          ...
          className={`text-lg md:text-xl ${isDark ? 'text-white/80' : 'text-warm-500'}`}
        >
          {subtitle}
        </motion.p>
      </div>
